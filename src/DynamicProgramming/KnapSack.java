	...
	public void solve(int[] weights, int[] values, int kpSize) {
		...
		for (int i = 0; i < weights.length; i++) {
			for (int j = 1; j < maxWeight; j++) {
				if (j < weights[i]) {
					kp[i + 1][j] = kp[i][j]; // weight array values are starting
												// from 0 index s0 here actuall
												// i = i+1
				} else {
					kp[i + 1][j] = Math.max(values[i] + kp[i][j - weights[i]],
							kp[i][j]); // weight array values are starting from
										// 0 index s0 here actuall i = i+1
				}
			}
		}
		// for(int i =0;i<weights.length;i++){
